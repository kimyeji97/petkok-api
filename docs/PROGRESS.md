...
> **07-31 에 "완료"로 기록한 Phase 0 가 실제로는 코드에 반영돼 있지 않았다.** 그걸 발견해 재실행한 날이다.

### 문서가 "완료"라고 말하는데 코드는 그대로였다

`/progress` 로 현황을 훑고 계획서를 코드와 대조하다 나왔다. PROGRESS·계획서 둘 다 "Phase 0 완료 ✅" 로 체크돼 있었는데 —

- `57f5ca1` 이 바꾼 것은 **문서 3개뿐**이었다
- ArchUnit 파일의 마지막 변경은 `efef567`(07-29)
- `allowEmptyShould(true)` 7건, `withOptionalLayers(true)`, 예외 #4 의 "임시" 주석이 **전부 그대로**
- 워킹 트리도 클린 — 로컬에조차 남아 있지 않았다

07-31 에 "껐는데도 8건 통과(`tests=7`+`tests=1`)" 를 확인한 것은 사실이지만 **그 상태가 커밋되지 않았다.** 검증과 커밋 사이에서 유실됐고, 체크박스가 켜져 있어 다음 사람은 그냥 지나갔을 것이다.

> **`/checkpoint` 가 체크박스를 켤 때 근거는 "돌려봤다"가 아니라 "커밋됐다"여야 한다.** 통과를 확인한 것과 그 상태가 저장소에 남는 것은 다른 일이다.
