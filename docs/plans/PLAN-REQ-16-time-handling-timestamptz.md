...
- [x] **④ Notion 역반영의 범위** → **전수 조사 완료 (2026-08-28).** 아래가 Phase 4 의 작업 목록이다.
      ...
      **2026-08-31 실행** — ⓐ 응답 12행 완료(리터럴 20개, 12행 전부 재조회 확인) · 요청 2행은 **고치지 않았다**(D9 가 `Z` 를 받으므로 유효) · ⓑ 완료 · ⓒ 완료.
      ...
      ⚠️ **조사가 「ERD 설계」 페이지도 빠뜨렸다 (2026-09-01, 여섯 번째 열거 오류).** 테이블 스키마 코드블록 9개(19곳) + 산문 2곳이 `timestamp`로 남아 있었다 — 사용자가 지적해 드러났다. ERD 페이지는 DB 탭 본문과 달리 일반 페이지 객체라 `notion-update-page`로 반영됐다.
      ✅ **탭 본문 2곳(2026-09-01 닫힘)** — API 탭 "날짜 포맷" 행, DB 탭 DDL 코드블록. API로 수정 불가해 사람 손으로 반영, `fetch` 재조회로 확인. DB 탭은 1차 붙여넣기가 줄 중간에서 잘려나가 깨졌던 것을 재조회로 잡아 재작업했다(PROGRESS 2026-09-01 참고).
      **ⓒ 레포 파생 요약** — `docs/specs/api-list.md` 의 "날짜 포맷: ISO 8601 (`2026-06-30`, `2026-06-30T15:00:00Z`)" 한 줄. `db-schema.md` 는 Phase 4 당시 갱신되지 않고 남아 있던 것을 2026-09-01 에 별도로 반영(PR #44)
- [x] **⑥ 엔티티 ↔ DB 실제 컬럼 타입 대조를 무엇으로 하는가** → **계약으로만 남긴다** (2026-08-28 확정). `범위 — 제외` 의 "자동 강제는 하지 않는다"와 같은 결정이다 — 뿌리가 같다(SQL 과 코드를 맞대볼 수단이 없다). Phase 4 에서 `CLAUDE.md` 에 올린다. **Testcontainers 도입은 별건**이고, 도입하면 REQ-10 의 keyset 경계·`@Transactional` 롤백까지 함께 닫히므로 그때 한꺼번에 다룬다.
      **2026-09-21 갱신 — REQ-18(Phase 1·2)이 이 갭을 메우기 시작했다.** `UserRepositoryContainerTest`(REQ-18-02)가 Testcontainers 실물 DB에 `User`를 저장→조회해 JPA Auditing 배선을 확인하고, 프로브(REQ-18-03)로 이 스모크 테스트가 정확히 이 계열의 결함(REQ-10 2026-09-07 실측과 동일한 예외 문구)을 잡아낸다는 것도 실측으로 확인했다. **다만 전 도메인·전 컬럼을 덮지는 않는다** — REQ-18 범위가 스모크 1건(User)으로 명시 한정돼 있어(`PLAN-REQ-18` § 범위 — 제외), keyset 경계 등 나머지는 여전히 사람 손 실측이다. `CLAUDE.md` 「시각 처리」 절 문구도 이 사실에 맞춰 정정했다(같은 커밋).
...
- [x] **Phase 4 — 문서 역반영** — 2026-08-31 실행, 2026-09-01 **탭 2곳 반영 확인으로 완료**
      ...
      완료 기준: 미결 ④ 가 닫힘 ✅(2026-09-01) · 원본과 코드가 어긋나는 곳 0건 ✅(같은 날, 탭 2곳 재조회 확인)
