...
> 출처: 2026-08-10 세션 · 작성: 2026-08-10 · 최종 갱신: 2026-09-18 · 상태: ✅ **완료 (Phase 1~3)**. 미결 0건(D3 예외 추가 시점 = REQ-10 Phase 0에서 2026-08-28 해소, 체크박스 정정은 2026-09-18)
...
- [x] **Notion 역반영 2건 — 2026-08-27 완료.** 「소스 구조」 §3(가드 반환 = `OwnedPetResponse` · 종 검증 = 각 하위 Service) · §6(소프트 딜리트 = 수동 파생 쿼리) 를 확정안으로 교체하고 갱신 이유를 붙였다. §11 예시 주석도 함께 정정. **§13 ① 스케치의 예외 목록은 REQ-10 에서 예외 3건이 코드에 들어간 뒤 맞춘다** — 문서가 코드보다 앞서지 않도록. API 로 수정 가능했다(탭이 아니라 일반 페이지)
- [x] **`DomainBoundaryTest` 의 D3 예외 3건(`business.pet.service` · `data.pet.dto` · `data.pet.enums`)을 언제 넣는가 — REQ-10 첫 하위 도메인 착수 시 (2026-08-27 등록).** Phase 3 에서 넣지 않은 이유: 지금은 소비자가 없어 **대상 없는 예외**가 되고, 이 프로젝트는 빈 규칙을 통과로 치지 않는다(`allowEmptyShould(false)` 와 같은 정신). 형태는 프로브로 검증됐으므로 REQ-10 계획서가 이 항목을 Phase 0 으로 가져간다. **2026-09-18 체크박스 정정 — REQ-10 Phase 0(2026-08-28, 커밋 `016c692`)에서 실제로 처리돼 해소됐다.** `docs/PROGRESS.md` 인덱스는 이미 2026-09-16에 "미결 0건"으로 정정돼 있었는데 이 계획서 원본 체크박스만 안 따라간 채 `/progress 예정,미결` 조회가 찾을 때까지 남아 있었다
- [x] **`GET /pets` 커서 페이지네이션 — 넣지 않는다 (2026-08-27 확정).** 원본 응답 `{"data":{"items":[...]}}` 에 `next_cursor` 가 없는 것을 재확인했다. AGENTS §5 의 커서 규약은 일반 원칙이지 이 행의 계약이 아니다. 필요해지면 **Notion `API I/F` 행을 먼저 고친 뒤** 추가한다
...
- [x] **회원 탈퇴 시 pets — 그대로 둔다, 접근 불가를 계약으로 명시 (2026-08-27 확정).** 원본 "회원 탈퇴 (소프트 딜리트). 204" 에 pets 언급 없음. `users.deleted_at` 만 찍히고 pets 행은 남지만 **API 로 닿는 경로가 없다** — 탈퇴 사용자는 필터(`UserStatusChecker`)가 401 로 막고, 재가입은 새 `users.id`(REQ-08 D1)라 옛 pets 에 소유권이 없다. 함께 소프트 딜리트하는 안은 `business/user → data/pet` 참조가 생겨 ArchUnit 예외 또는 포트가 필요한 설계 작업이라 기각. 남은 행의 정리는 만료 refresh 행 정리 배치와 같은 결정(스케줄러 도입)에 묶는다. 코드 변경 없음
- [x] **`DELETE /pets/{id}` "연관 기록 보존" = DB 행 보존, API 조회 불가 (2026-08-27 확정).** 원본은 "소프트 딜리트. 연관 기록(일지/식사/갤러리) 보존 · 204" 뿐이고 조회 가능 여부는 없다. 하위 도메인 엔드포인트가 전부 `/pets/{petId}/...` 아래이고 진입 시 `PetAccessGuard` 가 삭제된 펫에 404 를 내므로 **삭제된 펫의 하위 기록은 API 로 닿지 않는다.** 보존은 복구·감사용 DB 차원(§6 FK "캐스케이드 미지정")이다. REQ-10 에 추가 작업 없음 — 현재 가드 동작이 곧 계약
...
> ~~미결 4건이 남아 있다 (2026-08-10 기준).~~ **2026-08-27: 3건 해소(역반영 · 페이지네이션 · DELETE 보존), 남은 것은 D3 예외 추가 시점(REQ-10 Phase 0) 1건.** ①·①'·②는 Phase 1 프로브와 D3·D4 로 닫혔고, `PATCH`/`species`(D5)·소프트 딜리트(D6)도 Phase 2 착수 전에 닫았다. 남은 넷은 **전부 pet 도메인 밖으로 번지는 것들**이다 — Notion 역반영 2건 · 커서 페이지네이션 · 탈퇴 시 pets · `DELETE` 후 하위 기록 조회.
