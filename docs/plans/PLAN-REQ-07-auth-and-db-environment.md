...
> 출처: 2026-07-27 · 2026-07-29 세션 · 작성: 2026-07-27 · 최종 갱신: 2026-08-07 · 상태: ✅ 완료 (2026-08-07 카카오 로그인 왕복 확인 — **그 확인에서 응답 매핑 결함이 드러나 `bf41915` 로 수정**). 미결 0건 — 2026-08-27 에 2건 모두 해소(logout 범위 · 동시 refresh)
...
- [x] **동시 refresh 요청 — 감수한다 (2026-08-27 확정).** 유예 윈도우를 두지 않는다. Notion §7 은 "재사용 감지 → 전체 revoke" 만 규정하고 유예는 근거가 없으며, 유예는 탈취 탐지를 그만큼 약화시킨다. **refresh 직렬화(단일 인플라이트)는 클라이언트 책임**으로 명시한다 — 앱 구현 시 클라이언트 계약 문서에 적을 것. 코드 변경 없음
...
- [x] **`DELETE /auth/logout` 의 revoke 범위 — 사용자 전체 revoke 로 확정 (2026-08-27).** Notion `API I/F` 원본을 직접 읽었다: "Refresh Token 무효화. Request Body 없음. 204" — **범위 자체가 없다.** api-list 의 어긋난 두 줄은 둘 다 파생 요약이 덧붙인 것이었고 "해당 토큰" 쪽을 원본대로 정정했다. Body 가 없어 특정 토큰을 지목할 수 없으므로 전체 revoke(REQ-07-18)가 유일한 구현 해석. 기기별 로그아웃은 refresh 토큰을 Body 로 받는 **계약 변경**이 선행돼야 하며, 요구가 생기면 Notion 행을 먼저 고친다
...
> **결과 갱신: 2026-08-07 — 25건 전부 `✅`.** REQ-07-24·25 는 2026-08-07 카카오 왕복에서 터진 매핑 결함을 잡는 회귀 케이스로 뒤늦게 추가됐다 — ⚠️ **코드가 표보다 먼저 들어간 순서 위반**이었고(CLAUDE.md 는 표가 먼저다) 여기서 정리한다.
> (2026-07-30 시점: 23건 전부 `✅`, CI green @ `efde1f5`)
...
| REQ-07-24 | `KakaoTokenResponse` | `access_token` 이 매핑된다 (설정 없는 `ObjectMapper` 기준) | 회귀 | 2026-08-07 실측 — 카카오가 200 과 `access_token` 을 정상 반환했는데 전 필드가 `null` 이 됐다 | 4 | ✅ |
| REQ-07-25 | `KakaoUserResponse` | 중첩 `kakao_account.profile.nickname` · `profile_image_url` 매핑 | 회귀 | 〃 — 토큰 교환만 고치면 `getProfile` 에서 다시 터진다 | 4 | ✅ |
