...
| REQ-08-30 | `DELETE /users/me/profile-image` | 토큰 없는 요청 → 401 (상태코드 + `error.code` = `UNAUTHORIZED`) | 예외 | Phase 4 완료 기준 — "미인증 401" | 4 | ✅ |

> **2026-09-18 추가 — REQ-08-30 (`/testgen REQ-08`).** 위 "결과 갱신: 2026-08-27" 메모가 스스로 적어 둔 공백("완료 기준 중 '미인증 401'(Phase 4)은 케이스가 없어 테스트로는 고정되지 않았다")을 이번에 닫는다. `UserControllerWebMvcTest`를 대조한 결과 `REQ-15-05/06`은 `GET /users/me`만 미인증을 덮고 `DELETE /users/me/profile-image`는 204 케이스(`REQ-08-21`)뿐이었다 — 실제 공백이었다(REQ-10의 오판 사례와 다름).
> **결과 갱신: 2026-09-18 — REQ-08-30 `✅` (`/testrun REQ-08`).** 관련 테스트 7파일 47건 전부 실행 · 실패 0 · 수정 루프 0회(1차 통과) · 표 30행 ↔ 코드 29 ID 일치(`REQ-08-11`은 계획서가 명시한 대로 여전히 수동 검증, 코드 없음이 정상) · 근거 인용 표본 5건(REQ-08-06·10·20·29·30) 전건 원문 존재. **Phase 4 완료 기준 공백이 이걸로 완전히 닫혔다.**
