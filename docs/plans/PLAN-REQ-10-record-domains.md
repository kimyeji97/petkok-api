...
- [x] **"미래 날짜 불가"의 기준 타임존 — 2026-09-02 확정.** **KST(`Asia/Seoul`) 자정.** `ADR-0002`가 이 케이스를 이름까지 대며 이미 답해 뒀다 — "계산 — '오늘'·'당일'·'미래'·'일수'·'주기' 등 달력 판정은 전부 `Asia/Seoul` 자정 경계"(REQ-16). `LocalDate.now(Clock)`(KST 존 `Clock` 빈)으로 "오늘"을 구하고 `entry_date > 오늘` 이면 400. `fed_at`(Phase 3)과 같은 원칙.
      (원문:) `entry_date` 가 `date` 라 "오늘"이 KST 냐 UTC 냐에 따라 자정 전후 9시간이 갈린다
- [x] **`limit` 최대 50 vs `CursorRequest.MAX_LIMIT = 100` — 2026-09-02 확정.** **`DiaryService`에서 `CursorRequest`를 받은 뒤 `min(limit, 50)`으로 한 번 더 조용히 클램프한다.** `framework` 전역 상한(100)은 안 건드린다 — 범위·제외가 이미 "이번엔 framework 를 건드리지 않는다"고 못 박아 뒀다(이미 나간 weight·activity·feeding·shed 의 계약을 깰 수 없다). 400 거부가 아니라 클램프인 이유 — `CursorRequest` 자신이 100 초과를 클램프로 처리하는 것과 결을 맞춘다.
      (원문:) diary 만 50 으로 따로 보정할지, framework 상한을 50 으로 내릴지(다른 목록 원본은 상한 미명시)
- [x] **거꾸리 경고 — 2026-09-02 확정.** **Phase 5 에서 구현하지 않는다.** `condition_tag='거꾸리'` **저장**(D1 의 4종 중 하나)까지만 하고, 그 값을 집계해 경고를 응답에 싣는 파생 로직은 만들지 않는다 — **범위 — 제외**에 이미 있던 항목("원본 없이 만들지 않는다")이라 Phase 5 를 막는 미결이 아니었다. `API I/F` 에 응답 형태가 먼저 적혀야 재론한다.
      (원문:) 「소스 구조」 §8 에만 있고 `API I/F` 응답에 없다. 어느 엔드포인트에 어떤 형태로 실을지 원본에 먼저 적히기 전엔 만들지 않는다
...
- [x] **역반영 목록** (Phase 진행하며 사람이 Notion 에서) — 전 항목 완료(2026-09-03). ~~ⓐ 「소스 구조」§8·ERD·테이블 정의서의 `condition_tag` 7종 → 4종~~ (2026-09-03 완료 — 아래 참고) ~~ⓑ 테이블 정의서 `feeding_logs` 에 `food_size`~~ (2026-09-03 완료 — `varchar(1)` 행 추가, `amount_unit`·`is_refused` 사이) ~~ⓒ 활동·체중·탈피 목록 행에 `cursor`/`limit` 절~~ (2026-08-28 완료) ~~ⓓ 탈피 기록 "완료일은 시작일 이후" 삭제~~ (2026-09-03 완료 — API I/F 「탈피 기록」 콜아웃에서 제거 + 「비즈니스 규칙」 `BR-DIARY-05` 중복 행도 폐기 표시) ~~ⓔ 체중 경고 필드 명시~~ (2026-08-28 완료) ~~ⓕ 「소스 구조」 §13 ArchUnit 스케치에 예외 3건~~ (2026-08-28 완료 — callout + 스케치 ① 갱신, `fetch` 로 저장 확인) ~~ⓖ 급여 기록 callout "당일 시간만 허용" → "미래 시각 불가" ⓗ 거식 스트릭 응답 예시의 자기모순 ⓘ 「소스 구조」 §10 에 `FEATURE_NOT_SUPPORTED_SPECIES` 추가 ⓙ 급여 기록 행에 `food_size` 처리 절~~ (2026-09-01 완료 — `fetch` 재조회로 4곳 전부 확인. ⓘ의 "`SHED_NOT_SUPPORTED_SPECIES` 제거"는 대상이 애초에 Notion에 없어 추가만 했다, 위 미결 참고) — Phase 3 착수 전 원본 정리 끝. (시각 예시 `…Z` → `+09:00` 은 REQ-16 Phase 4 소관, 완료됨) ~~ⓚ 테이블 정의서 `shed_records` 비즈니스 규칙의 `is_assisted ↔ condition_tag '탈피도와줌'` 연계 문구 삭제~~ (2026-09-03 완료 — 폐기 사유를 남기고 대체)
      ✅ **ⓐ 완료(2026-09-03).** 「ERD 설계」`diary_entries` 코드블록·비고 2곳, 「소스 구조」§4 표·§8 확정문구·"다음 단계" TODO 4곳 — 총 6곳은 API `update_content`로 고쳤다(이 두 문서는 값 목록이 일반 텍스트/코드블록이라 읽기·쓰기 정상 동작). 「테이블 정의서」`diary_entries.condition_tag` 셀만 API로 원문을 못 읽어(첫 값 + `\\` 에서 끊김, CLAUDE.md 승격 `9d3863b`) 사람이 직접 고쳤다 — `fetch` 재조회로 `"개체 상태 태그. (정상 \| 활발 \| 거꾸리 \| 구토)"` 확인, 다시 쓰면서 저장 형태 자체가 정상으로 바뀌어 이제 API로도 읽힌다. 같은 문서의 `species`·`gender`·`provider`·`activity_type` 셀은 여전히 같은 증상이지만 값 자체는 이미 정확해 손대지 않았다.
...
>
> ✅ **정정 — 2026-09-18, `/testgen REQ-10` 재조사.** 위 발견은 오판이었다. `WeightDtoContractTest`·`ActivityDtoContractTest`·`FeedingDtoContractTest`·`ShedDtoContractTest`·`DiaryDtoContractTest` 다섯 파일 전부 **각 Phase의 원래 구현 커밋(2026-08-28~09-02)에서부터 이미 존재**했고(`git log --follow` 대조 — 수정 이력 없음), 지금도 5/5 전부 통과한다(`build/test-results/test/*.xml` 재확인). 2026-09-10 시점에 왜 "없다"로 잘못 판단했는지는 이번에도 추적하지 않았다 — `--tests` 글롭·grep 패턴이 실제 결과를 가리는 이 레포의 기존 함정(`CLAUDE.md` 「로컬 검증」)과 같은 유형일 가능성이 있다는 추정만 남긴다. **표의 `✅`는 그대로 유효하다** — 다섯 도메인을 다음에 건드릴 때 반영 검토할 것은 없다.
>
