...
- **개발 플로우는 여섯 커맨드 사이클로 돈다 (2026-09-02 계약 승격).** REQ 단위 작업은 아래 순서로 진행하고, 각 커맨드의 책임 밖 일을 그 커맨드 안에서 하지 않는다
	...
	- **미결 질문이 열린 Phase는 착수하지 않는다** — `/implement` 게이트가 거른다. 원본(Notion)에 답이 없으면 추측으로 채우지 말고 `/workplan`으로 돌아가 미결로 남긴다(§0)
