...
- **REQ 단위 작업 순서 — 계획서 → 검증 계약(테스트) → 구현 → 판정 → 기록.** 도구와 무관한 원칙이다: 테스트 코드가 구현보다 먼저 오고, **Phase 1개 = 커밋 1개**이며, 판정하는 손은 구현을 고치지 않는다. 미결 질문이 열린 Phase는 착수하지 않는다(§0 — 원본에 답이 없으면 추측으로 채우지 않는다). Claude Code에서는 이 순서가 슬래시 커맨드 여섯 개로 고정돼 있다 → [`CLAUDE.md`](CLAUDE.md) 「개발 플로우」
...
- ⚠️ **PR이 머지되고 원격 브랜치가 삭제돼도 로컬 브랜치는 스스로 없어지지 않는다.** `git status`·`git branch --show-current` 어디에도 이상 신호가 없고, 그 위에 새 커밋을 얹어도 git이 경고 없이 받아준다 — 이미 머지된 옛 커밋들 위에 새 작업이 쌓인 채로 존재하게 된다. 2026-09-01 실제로 발생했다(PR #43 머지·브랜치 삭제 후 `docs/req16-phase4-backfill` 로컬 브랜치에 그대로 커밋). `git fetch --prune` 후 `git branch -vv`에서 해당 브랜치가 `[origin/… : gone]`으로 보일 때만 드러난다
	...
	- ⚠️ **더 앞 단계의 원인 — `main`에 직접 커밋하고 바로 안 미는 습관.** `/checkpoint`처럼 문서만 고치는 작업은 종종 `main`에서 그대로 커밋되는데, **그 자리에서 바로 푸시하지 않으면** 다음에 그 위에서 새 브랜치를 파는 순간 로컬 `main`이 `origin/main`과 갈라진다. 2026-09-01 같은 세션에서 **두 번 재발**(PR #43 이후·PR #47 이후) — 매번 복구는 됐지만(`git diff` 양방향으로 로컬에만 있는 내용이 없는지 확인 후 `git reset --hard origin/main`), 재발 자체가 습관 문제라는 신호다. **`main`에 직접 커밋했으면 그 자리에서 바로 `git push`한다.**
