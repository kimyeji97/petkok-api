...
- **`src/test`는 ArchUnit 구조 규칙 8개 + REQ-07 검증 계약(auth)뿐이다.** auth 외 도메인 로직 테스트는 아직 없고, 구조 규칙 상당수는 대상 코드가 없어 **빈 집합을 대상으로 통과**한다(`allowEmptyShould(true)`). `./gradlew test` 통과를 "기능이 검증됐다"로 넓혀 보고하지 말 것 — **커버된 케이스는 계획서 `## 검증 계약` 표가 전부다**
