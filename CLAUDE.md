...
- **앱을 띄우려면 `.env`가 필요하다** — `application.yml`이 `spring.config.import: optional:file:.env[.properties]`를 갖고 있어 **`./gradlew bootRun`만으로 읽힌다**(2026-08-28 실측 확인 — `set -a && . ./.env` 로 직접 주입할 필요 없음). `DB_PASSWORD`에는 기본값이 없어 `.env`가 없거나 값이 비면 기동이 막힌다(의도적)
...
- **신규 시각 컬럼은 `timestamptz`, 엔티티 필드는 `OffsetDateTime`.** `timestamp` + `LocalDateTime` 쌍은 **저장된 값만으로 순간이 결정되지 않는다** — 세션 TZ 에 따라 같은 값이 다른 순간이 되고, 그 어긋남은 에러 없이 나타난다. 날짜만 있는 값(`entry_date`·`shed_date`·`measured_at`·`birthday`·`adoption_date`·`taken_at`)은 예외다. 타임존 개념이 없으므로 `date`·`LocalDate` 를 유지한다
	...
	- **이 두 줄은 같이 읽어야 뜻이 산다.** 위 규칙을 어겨도 기동이 안 막히는 이유가 아래 사실이다. 즉 **엔티티만 바꾸고 마이그레이션을 빠뜨리면(또는 그 반대) 조용히 통과한다.** 마이그레이션 텍스트 검사(`V3TimestamptzMigrationTest`)와 엔티티 타입 검사(`TimeFieldTypeContractTest`)는 **둘 다 SQL 과 DB 실물을 맞대보지는 않는다.** **REQ-18(2026-09-21)이 Testcontainers 를 도입해 이 구멍을 메우기 시작했다** — `UserRepositoryContainerTest`가 실물 DB에 `User`를 저장→조회해 JPA Auditing 배선을 확인하고, `JpaAuditingConfig.dateTimeProviderRef`를 일부러 빼는 프로브로 이 테스트가 REQ-10 2026-09-07 실측과 같은 결함을 실제로 잡아낸다는 것까지 확인했다(REQ-16 미결 ⑥ 해소, `PLAN-REQ-18` 참고). **다만 스모크 1건(User) 범위라 전 도메인·전 컬럼을 덮지 않는다** — keyset 경계 등 나머지는 여전히 사람 손 실측이다
		- ⚠️ **시각 타입 프로브를 짤 때 변환 방향을 헷갈리지 말 것 — 대칭이 아니다(REQ-18-03 실측, 2026-09-21).** 엔티티 필드를 `OffsetDateTime`→`LocalDateTime`으로 "되돌리면" FAIL할 거라 예상하기 쉬운데, **실제로는 통과한다** — Spring `ObjectToObjectConverter`가 `LocalDateTime.from(OffsetDateTime)`으로 오프셋만 버리고 조용히 변환한다. **진짜 결함(REQ-10 2026-09-07)은 반대 방향**이다: 기본 `DateTimeProvider`(`LocalDateTime`만 반환) → `OffsetDateTime` 필드. `LocalDateTime`엔 오프셋 정보가 없어 `OffsetDateTime.from(LocalDateTime)`이 예외로 죽는다. 프로브를 심을 땐 어느 쪽이 정보를 잃는 변환인지(항상 그 방향만 실패한다) 먼저 확인할 것
- **`now` 는 `Clock` 을 주입받는다. 무인자 `now()` 를 부르지 않는다** — `LocalDateTime`·`OffsetDateTime`·`LocalDate`·`Instant`·`ZonedDateTime` 5종 모두. JVM 기본 TZ 에 암묵 의존해 배포 환경에 `TZ` 가 없으면 **에러 없이** 9시간 어긋난다. ArchUnit `REQ_16_10_NO_DIRECT_NOW` 가 `business`·`framework` 에서 강제한다(`data` 는 범위 밖 — 엔티티에는 빈을 주입할 수 없다)
	- ⚠️ **`data` 가 범위 밖인 것과, JPA Auditing 이 안전한 것은 다른 얘기다.** `@CreatedDate`/`@LastModifiedDate` 는 엔티티 코드가 아니라 Spring Data 가 내부적으로 부르는 기본 `DateTimeProvider` 를 쓰는데, 이건 `LocalDateTime.now()` 만 반환한다. 엔티티 필드가 `OffsetDateTime` 이면 Spring Data 가 변환을 못 해 **저장 자체가 예외로 죽는다**(`Cannot convert unsupported date type ... LocalDateTime to ... OffsetDateTime`) — ArchUnit 규칙의 사각지대를 정확히 파고드는 경로다. `JpaAuditingConfig` 가 `TimeConfig` 의 `Clock` 으로 `OffsetDateTime.now(clock)` 을 공급하는 `DateTimeProvider` 빈을 두고 `@EnableJpaAuditing(dateTimeProviderRef = ...)` 로 지정해 막는다. `./gradlew test` 는 DB 를 안 타 이 경로를 검증하지 못하므로(`CLAUDE.local.md`), 이 배선을 건드리거나 `BaseCreatedEntity`/`BaseTimeEntity` 를 상속하는 새 엔티티를 추가할 때는 **실제 DB 로 확인할 것**. 2026-09-07 REQ-10 Phase 1·2 로컬 DB 실측에서 카카오 자동가입(`users` 최초 INSERT)부터 500 으로 막혀 발견했다(PR #51) — 걸리면 모든 도메인의 모든 최초 INSERT 가 똑같이 죽는다
...
- **「설계」 섹션의 DB·API 탭에서 API로 수정할 수 없는 건 "탭 안에서 별도 페이지로 링크된 것"뿐이다 — 탭에 직접 박힌 콘텐츠(코드블록 등)는 된다.** `notion-update-page`를 탭이 링크하는 페이지(예: 「ERD 설계」)에 직접 쓰면 `validation_error: Object ... is not a page or database`로 거부한다 — `fetch`로 읽히고 URL도 페이지처럼 생겼지만 쓰기가 안 되는 객체(탭)다. **읽기는 되는데 쓰기만 막히면 권한이 아니라 객체 타입을 의심할 것.**
	- **단, 탭 안의 데이터베이스(`API I/F` 등)에 행을 추가하는 것은 된다** — `notion-create-pages` 에 `parent.data_source_id`(`collection://…`, `fetch` 로 얻는다)를 주면 생성된다(2026-08-27 실측, `DELETE /users/me/profile-image` 행). 기존 행 본문 수정(`update-page`)도 된다.
	- ⚠️ **정정(2026-09-21) — DB 탭의 `DDL (PostgreSQL)` 코드블록은 별도 페이지가 아니라 최상위 페이지(`팻콕(PetKok)` 메인 페이지, `389b81b5…`) 본문에 `<tabs><tab>` 으로 직접 박힌 콘텐츠다.** 그 최상위 페이지 `page_id`로 `update_content`를 쓰면 **정상 반영된다** — REQ-17 `measured_at`→`measured_date`·`taken_at`→`taken_date` 리네임을 이렇게 반영, `fetch` 재조회로 확인(`page_last_edited_at` 갱신도 확인). 예전엔 "DB·API 탭은 API로 수정할 수 없다"고 일반화해 시도조차 안 하고 사람 손으로 넘겼는데, 그건 **탭이 링크하는 별도 페이지**(예: 「ERD 설계」)에만 해당하는 제약이었다. **대상이 탭에 인라인으로 박힌 콘텐츠인지, 탭이 링크하는 별도 페이지인지부터 구분하고, 전자면 최상위 페이지에 `update_content`를 먼저 시도할 것** — "사람이 직접"으로 넘기는 건 후자로 확인된 다음이다
...
- **콜아웃·블록 태그(`<callout>` 등)를 HTML 엔티티로 이스케이프하면 안 된다.** `replace_content`로 페이지를 통째로 재작성할 때 `<`·`>`를 `&lt;`·`&gt;`로 쓰면(복사·붙여넣기 과정에서 자동 이스케이프되기 쉽다) 도구는 성공을 반환하지만 **블록이 아니라 리터럴 텍스트로 저장된다** — 페이지를 열면 `<callout icon="📌"...>` 문자열이 그대로 보인다. 실측(2026-09-18, 「R2 업로드 URL 발급」 페이지): `replace_content`에서 한 번 발생, `update_content`로 리터럴 `<`·`>`를 써서 재수정 후 `fetch` 재조회로 정상 렌더 확인. **콜아웃·블록 태그는 항상 리터럴 꺾쇠로 쓸 것** — 성공 응답만 보고 넘기지 말고 재조회로 렌더링을 확인한다
- **테이블 셀의 "값 목록" 서술이 첫 값 이후 `\` 에서 잘려 반환될 수 있다.** 「테이블 정의서」의 `condition_tag`·`species`·`gender`·`provider`·`activity_type` 값 목록 셀이 `fetch`·`search` 양쪽에서 전부 `첫값 \\` 로 끝나고 나머지 값은 안 실린다(2026-09-03 실측, 재조회로도 재현 — 일회성 아님). §2 "코드블록 이스케이프 문자열"과 증상은 비슷하지만 **이번엔 부분 치환은커녕 전체 원문조차 API로 못 읽는다** — `old_str`을 만들 재료가 없다. `replace_content` 전체 교체도 정확한 원문을 모르면 못 쓴다. **사람이 Notion UI에서 그 셀을 지우고 값을 다시 입력하면 저장 형태 자체가 정상화된다** — `condition_tag` 셀 실측(2026-09-03): 사람이 재입력한 뒤 `fetch` 재조회에서 끊김 없이 전부 읽혔다. 값을 못 읽어도 "고친 뒤 다시 읽어보기"까지는 시도할 것 — 깨진 저장 형태 자체가 고쳐질 수 있다. 값 목록이 아닌 콜아웃·불릿(단일 문단)은 이 문제가 없다(같은 세션에서 API I/F 콜아웃·비즈니스 규칙 프로퍼티·새 테이블 행 추가·ERD·소스구조 문서의 같은 값 목록 사본은 전부 API로 정상 동작 확인 — **저장 위치(테이블 셀)가 문제이지 값 자체나 문서 종류의 문제가 아니었다**)
