 ...
 *
 * <p>{@code @Size} 는 {@code null} 을 통과시킨다 — 그래서 D3 과 충돌하지 않는다. 길이만 막는 이유는 {@code varchar(100)} 이
 * 스키마에서 확정된 사실이기 때문이고, 이게 없으면 101자 요청이 {@code DataIntegrityViolationException} 으로 올라와 <b>400 이 아니라
 * 500</b> 이 된다 (D7). 최소 길이·트림·빈 문자열은 원본 근거가 없어 미결로 남겼다.
 *
 * <p>검증 계약 REQ-08-06 · REQ-08-07.
 */
public record UserUpdateRequest(
    @Size(max = 100) String nickname, @Size(max = 500) String profileImageUrl) {}
