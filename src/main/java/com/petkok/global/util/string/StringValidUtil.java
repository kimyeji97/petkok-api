...
public class StringValidUtil {
  ...
  private StringValidUtil() {}

  // URL 패턴
  private static final Pattern PATTERN_URL =
      Pattern.compile(
          "^((http(s)?|ftp):\\/\\/)?+((([^\\/:]\\S+\\.)?([[^\\/.:]&&\\S]+)?+)(:(\\p{Digit}+))?+)?+(\\/([\\S&&[^\\?#]])*)?(\\??(&?[\\S&&[^&=#]]+=?[\\S&&[^&=#]]+)*)?(#.*)?$");
  // HEX color 패턴
  ...
  private static final Pattern BRN_PATTERN =
      Pattern.compile("([0-9]{3})(-?)([0-9]{2})(-?)([0-9]{5})");
