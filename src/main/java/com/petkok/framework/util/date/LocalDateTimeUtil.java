package com.petkok.framework.util.date;
...
public class LocalDateTimeUtil {
  ...
  public static LocalDateTime getLocalDateTime(long time, ZoneId zoneId) {
    return LocalDateTime.ofInstant(
        Instant.ofEpochMilli(time), zoneId == null ? ZoneId.systemDefault() : zoneId);
  }
  ...
  /**
   * 특정 범위 안에서 분 단위 Date 객체를 리스트로 구한다.
   *
   * @param start 시작 Date
   * @param end 끝 Date
   * @param minutes 분 단위
   * @return 분 단위로 나눈 Date 리스트
   */
  public static List<LocalDateTime> getDateTimesInMinutes(
      LocalDateTime start, LocalDateTime end, int minutes) {
  ...
  public static int diff(ChronoUnit field, LocalDateTime date1, LocalDateTime date2) {
    return switch (field) {
      ...
      case HOURS ->
          LocalTime.of(0, 0).plusSeconds(Duration.between(date1, date2).getSeconds()).getHour();
      case MINUTES ->
          LocalTime.of(0, 0).plusSeconds(Duration.between(date1, date2).getSeconds()).getMinute();
      case SECONDS ->
          LocalTime.of(0, 0).plusSeconds(Duration.between(date1, date2).getSeconds()).getSecond();
      case NANOS ->
          LocalTime.of(0, 0).plusSeconds(Duration.between(date1, date2).getSeconds()).getNano();
