package com.petkok.framework.util.map;
...
/**
 * MultiKeyHashMap provides to store values with two level hierarchy of keys, super key (K1) and sub
 * key (K2). The objects are inserted using super and sub keys. It is not mandatory to use both keys
 * as hierarchy, user can use two keys to store the values and use either of the key to retrieve it.
 *
 * <p>import - https://mvnrepository.com/artifact/com.google.guava/guava
 *
 * @author Prathab K
 */
public class MultiKeyHashMap<K1, K2, V> {
  ...
  public static final MultiKeyHashMap EMPTY_MAP =
      new MultiKeyHashMap(ImmutableMap.builder().build());
   ...
   *
   * @param k1 key1 (super-key)
   * @return HashMap structure contains the values for the key k1 if exists or <tt>null</tt> if does
   *     not exists
   */
  ...
  public Set<MultiKey<K1, K2>> multiKeys() {
    return mkMap.entrySet().stream()
        .flatMap(
            k1Entry ->
                k1Entry.getValue().keySet().stream()
